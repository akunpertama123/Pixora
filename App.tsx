...
const App: React.FC = () => {
  ...
  const handleLogin = useCallback((email: string, password_param: string) => {
    setAuthError(null);
    const userInList = users.find(u => u.email === email);
    const isAdminLoginAttempt = email === ADMIN_EMAIL && password_param === ADMIN_PASSWORD;

    if (isAdminLoginAttempt) {
        const adminUser: User = { id: userInList?.id || 'admin_user', email: ADMIN_EMAIL, role: 'admin' };
    ...
    if (userInList && userInList.password === password_param && userInList.role === 'buyer') {
      const loggedInUser: User = {...userInList, password: "" }; 
      setCurrentUser(loggedInUser);
      setCurrentView('buyer');
      localStorage.setItem('currentUser', JSON.stringify(loggedInUser));
      const storedCart = localStorage.getItem(`cart_${loggedInUser.id}`);
      setCartItems(storedCart ? JSON.parse(storedCart) : []);
    } else {
  ...
  const handleRegister = useCallback((email: string, password_param: string) => {
    ...
    const newUserWithPassword: User = { id: Date.now().toString(), email, password: password_param, role: 'buyer' };
    const updatedUsers = [...users, newUserWithPassword];
    setUsers(updatedUsers);
    localStorage.setItem('users', JSON.stringify(updatedUsers));
    
    const loggedInUser: User = { id: newUserWithPassword.id, email: newUserWithPassword.email, role: 'buyer' };
    setCurrentUser(loggedInUser);
    setCurrentView('buyer');
    localStorage.setItem('currentUser', JSON.stringify(loggedInUser));
    setCartItems([]);
